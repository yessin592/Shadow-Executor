// ================================================================
// SHADOW EXECUTOR V99 - MAIN (NO RESOURCE DEPENDENCY)
// ================================================================
...
bool InjectDLL() {
    ...
    // Load Executor.dll from the same folder
    HMODULE hDll = LoadLibraryA("Executor.dll");
    if (!hDll) {
        MessageBoxA(NULL, "Failed to load Executor.dll!\nMake sure it's in the same folder.", "Error", MB_OK);
        SetWindowTextA(hStatus, "❌ DLL not found!");
        return false;
    }

    // Call an exported function to start the injection
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        if (LOWORD(wParam) == 3) {
            SaveCurrentTabContent();
            // Send script to pipe (if DLL is injected)
